...
CSV_PATH = BASE_DIR / "startrack_raw.csv"
...
def load_csv_events(csv_path):
    """
    Load CSV data, process, interpolate and return list of CSVEvent objects.
    """
    df = pd.read_csv(csv_path, sep='\s+')
    
    

    # Apply minimum radius floors
    df['radius1'] = apply_min_radius(df['Ra'])
    df['radius2'] = apply_min_radius(df['Rb'])
    df['stype1'] = df['Ka'].astype(int)
    df['stype2'] = df['Kb'].astype(int)
    df['time'] = df['tstart'].astype(float)
    df['m1'] = df['Ma'].astype(float)
    df['m2'] = df['Mb'].astype(float)

    # Event string column
    df['eventString'] = [f"{csv_stellarTypeName(k1)} + {csv_stellarTypeName(k2)}"
                         for k1, k2 in zip(df['stype1'], df['stype2'])]

    # Mass transfer rates for visualization
    df['dm1'] = [-abs(mt) if mt > 0 else 0.0 for mt in df['mt']]
    df['dm2'] = [abs(mt) if mt > 0 else 0.0 for mt in df['mt']]

    df = insert_extra_interpolations(df)
    ...
    df_final = pd.concat(phases, ignore_index=True)
    #df_final['a'] *= 2  # Double the semimajor axis everywhere


    df_final = df_final.dropna(subset=['stype1', 'stype2'])
    output_csv_path = BASE_DIR / "startrack_processed.csv"
